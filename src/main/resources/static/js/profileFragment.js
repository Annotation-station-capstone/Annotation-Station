        ...
        success:
            function (data) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    $(cardContainer).append("<div class='col-lg-3 col-md-3 mb-3 collectionCards card d-flex align-items-stretch shadow-4 rounded-5'><div class='card shadow-4 rounded-5 container'>" + image + cardBody + '<p class=\'card-text CSections\'> Sections: ' + sectionsHtml + '</p>' +  "</div> " + "<div class='row' ><div class='btn-group m-1' role='group' aria-label='Basic example'><button style='border-right: solid #444444' class='col-sm btn btn-select CButton'  id='edit" + [i] + "' data-user='" + collection_user_id + "' value='" + collection_id + "'>Edit</button><button class='col-sm btn btn-select CButton'  id='delete" + [i] + "' data-user='" + collection_user_id + "' value='" + collection_id + "'>Delete</button>" +"</div></div>");
